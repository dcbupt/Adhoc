...
### 连通性矩阵更新策略
本节点根据自身天线接收到的信息判断周边可通信的节点，更新矩阵中与自身节点相关的矩阵元素，并通过接收别的节点发布的连通性矩阵，更新其它节点间的连通性元素信息。并在自身时隙时，将更新后的连通性矩阵发布出去，供其它节点更新。<BR>
连通矩阵：<BR>
对于10个节点的Adhoc网络，用10X10的矩阵描述网络拓扑。connect_matrix[i][j]=1表示节点i和节点j连通，为0表示不连通。因为矩阵是对角线对称的，所以只需要45bit就可以描述连通矩阵。<BR>
每当收到邻居节点的宽波束组网消息，解析其连通矩阵字段，还原得到邻居节点的连通矩阵，然后与本地连通矩阵的对应位置做如下操作更新本地连通矩阵：<BR>
1、如果本地连通矩阵对应位为1，则做与操作更新本地对应位<BR>
2、如果本地连通矩阵对应位为0，则做或操作更新本地对应位<BR>
...
## 网络层
### 计算路由算法
网络层路由算法，基于下一跳路径最短原则实现局部最优路由的DijSktra算法的设计：<BR>
（1）	初始化dist[]数组，下标代表节点ID，其中本节点ID对应下标的值置0，其余置一个较大的数，dist数组描述目的节点到本节点的跳数<BR>
（2）	遍历dist数组，找到min对应的下标i（节点ID），通过连通矩阵找到该节点的邻居节点j，如果dist[j]>dist[i]+1，置dist[j]=dist[i]+1，pre_node[j]=i。如果dist[j]=dist[i]+1，通过i和j迭代出i路由和j之前的路由对应的下一跳ID，分别计算本节点到这两个下一跳ID的距离，如果新路由到下一跳ID的距离近，pre_node[j]=i，即修改到j的路由为新路由。<BR>
（3）	重复步骤2，直到所有dist数组对应的节点ID都被访问。<BR>
（4）	根据节点ID和pre_node数组迭代得到该节点ID的路由信息（下一跳节点ID）并将路由信息添加到路由表<BR>
